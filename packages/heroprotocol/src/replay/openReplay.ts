...
export async function openReplay(
  ...
  if (userData === undefined || userData.byteLength === 0) {
    throw new InvalidReplayError('the archive has no user-data header, so it carries no replay header');
  }

  // --- header: bootstrap protocol, then the build it names ---
  const bootstrap = await loadProtocol(source, BOOTSTRAP_BUILD);
  if (bootstrap === undefined) throw new ProtocolNotFoundError(BOOTSTRAP_BUILD, 'the bootstrap protocol is missing from the source');
  let header: ReplayHeader;
  try {
    header = finish(bootstrap.decodeHeader(userData), strings) as ReplayHeader;
  } catch (e) {
    throw new InvalidReplayError(`the replay header did not decode: ${e instanceof Error ? e.message : String(e)}`);
  }
  ...
  {
    const outcome = await decodeWithCandidates(source, candidates, (protocol) => finish(protocol.decodeHeader(userData), strings));
  ...
  for (const section of ['details', 'initData'] as const) {
    ...
    if (bytesFor === null) {
      sections[section] = { status: 'failed', attempts: [], error: `${SECTION_FILES[section]} is not in the archive` };
      continue;
    }
    progress?.({ section, current: 0, total: 1 });
    const outcome = await decodeWithCandidates(source, candidates, (protocol) => {
      const value = section === 'details' ? protocol.decodeDetails(bytesFor) : protocol.decodeInitData(bytesFor);
  ...
  if (wanted.has('attributes')) {
    ...
    if (a === null) {
      sections.attributes = { status: 'failed', attempts: [], error: `${SECTION_FILES.attributes} is not in the archive` };
    } else {
      try {
  ...
  for (const section of ['trackerEvents', 'messageEvents', 'gameEvents'] as const) {
    ...
    const outcome = await decodeEventsWithCandidates(source, candidates, section, bytesFor, drop, strings, progress);
    sections[section] = outcome.diagnostic;
  ...
  for (const d of Object.values(sections)) if (d.protocol !== undefined && d.status !== 'skipped') used.set(d.protocol, (used.get(d.protocol) ?? 0) + 1);
  used.delete(BOOTSTRAP_BUILD);
  const [protocol, provenance] =
    [...used.entries()].sort((a, b) => b[1] - a[1]).map(([p]) => [p, candidates.find((c) => c[0] === p)?.[1] ?? 'nearest'] as const)[0] ??
...
function checkDetails(details: ReplayDetails): void {
  ...
  for (const p of details.m_playerList ?? []) {
    if (typeof p.m_name !== 'string' || hasReplacementChar(p.m_name)) throw new Error('a player name is not valid text');
  }
}
...
async function decodeWithCandidates(
  ...
  return { diagnostic: { status: 'failed', attempts, error: attempts.at(-1)?.error ?? 'no candidate protocol' } };
}
...
async function decodeEventsWithCandidates(
  ...
  const attempts: DecodeAttempt[] = [];
  let best: { events: RawEvent[]; build: number; provenance: Provenance; failedAt: NonNullable<SectionDiagnostic['failedAt']>; error: string } | undefined;
  ...
  for (const [build, provenance] of candidates) {
    ...
    const stream =
      section === 'trackerEvents' ? protocol.trackerEvents(bytes, opts)
      : section === 'messageEvents' ? protocol.messageEvents(bytes, opts)
    ...
    try {
      ...
      return { events, diagnostic: { status: 'ok', protocol: build, provenance, attempts, eventsDecoded: events.length } };
    } catch (e) {
      ...
      if (events.length > 0 && (best === undefined || events.length > best.events.length)) {
        best = { events, build, provenance, error, failedAt: { usedBits, totalBits: total, lastGameloop } };
      }
    }
  }
