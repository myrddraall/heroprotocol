// Generated by tools/generate-protocols.mts from Blizzard/heroprotocol (protocol41150.py, blob 58cd43c25c0e3064303b0821fe43fc674644e5e7). Do not edit.
import type { ProtocolDefinition } from '../definition.js';

const protocol: ProtocolDefinition = {
  build: 41150,
  typeinfos: [
    {
      k: 'int',
      bounds: [0, 7],
    },
    {
      k: 'int',
      bounds: [0, 4],
    },
    {
      k: 'int',
      bounds: [0, 5],
    },
    {
      k: 'int',
      bounds: [0, 6],
    },
    {
      k: 'int',
      bounds: [0, 14],
    },
    {
      k: 'int',
      bounds: [0, 22],
    },
    {
      k: 'int',
      bounds: [0, 32],
    },
    {
      k: 'choice',
      bounds: [0, 2],
      choices: {
        '0': ['m_uint6', 3],
        '1': ['m_uint14', 4],
        '2': ['m_uint22', 5],
        '3': ['m_uint32', 6],
      },
    },
    {
      k: 'struct',
      fields: [['m_userId', 2, -1]],
    },
    {
      k: 'blob',
      bounds: [0, 8],
    },
    {
      k: 'int',
      bounds: [0, 8],
    },
    {
      k: 'struct',
      fields: [
        ['m_flags', 10, 0],
        ['m_major', 10, 1],
        ['m_minor', 10, 2],
        ['m_revision', 10, 3],
        ['m_build', 6, 4],
        ['m_baseBuild', 6, 5],
      ],
    },
    {
      k: 'int',
      bounds: [0, 3],
    },
    {
      k: 'bool',
    },
    {
      k: 'array',
      bounds: [16, 0],
      typeid: 10,
    },
    {
      k: 'optional',
      typeid: 14,
    },
    {
      k: 'blob',
      bounds: [16, 0],
    },
    {
      k: 'struct',
      fields: [
        ['m_dataDeprecated', 15, 0],
        ['m_data', 16, 1],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_signature', 9, 0],
        ['m_version', 11, 1],
        ['m_type', 12, 2],
        ['m_elapsedGameLoops', 6, 3],
        ['m_useScaledTime', 13, 4],
        ['m_ngdpRootKey', 17, 5],
        ['m_dataBuildNum', 6, 6],
        ['m_fixedFileHash', 17, 7],
      ],
    },
    {
      k: 'fourcc',
    },
    {
      k: 'blob',
      bounds: [0, 7],
    },
    {
      k: 'int',
      bounds: [0, 64],
    },
    {
      k: 'struct',
      fields: [
        ['m_region', 10, 0],
        ['m_programId', 19, 1],
        ['m_realm', 6, 2],
        ['m_name', 20, 3],
        ['m_id', 21, 4],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_a', 10, 0],
        ['m_r', 10, 1],
        ['m_g', 10, 2],
        ['m_b', 10, 3],
      ],
    },
    {
      k: 'int',
      bounds: [0, 2],
    },
    {
      k: 'optional',
      typeid: 10,
    },
    {
      k: 'struct',
      fields: [
        ['m_name', 9, 0],
        ['m_toon', 22, 1],
        ['m_race', 9, 2],
        ['m_color', 23, 3],
        ['m_control', 10, 4],
        ['m_teamId', 1, 5],
        ['m_handicap', 0, 6],
        ['m_observe', 24, 7],
        ['m_result', 24, 8],
        ['m_workingSetSlotId', 25, 9],
        ['m_hero', 9, 10],
      ],
    },
    {
      k: 'array',
      bounds: [0, 5],
      typeid: 26,
    },
    {
      k: 'optional',
      typeid: 27,
    },
    {
      k: 'blob',
      bounds: [0, 10],
    },
    {
      k: 'blob',
      bounds: [0, 11],
    },
    {
      k: 'struct',
      fields: [['m_file', 30, 0]],
    },
    {
      k: 'optional',
      typeid: 13,
    },
    {
      k: 'int',
      bounds: [-9223372036854776000, 64],
    },
    {
      k: 'blob',
      bounds: [0, 12],
    },
    {
      k: 'blob',
      bounds: [40, 0],
    },
    {
      k: 'array',
      bounds: [0, 6],
      typeid: 35,
    },
    {
      k: 'optional',
      typeid: 36,
    },
    {
      k: 'array',
      bounds: [0, 6],
      typeid: 30,
    },
    {
      k: 'optional',
      typeid: 38,
    },
    {
      k: 'struct',
      fields: [
        ['m_playerList', 28, 0],
        ['m_title', 29, 1],
        ['m_difficulty', 9, 2],
        ['m_thumbnail', 31, 3],
        ['m_isBlizzardMap', 13, 4],
        ['m_restartAsTransitionMap', 32, 16],
        ['m_timeUTC', 33, 5],
        ['m_timeLocalOffset', 33, 6],
        ['m_description', 34, 7],
        ['m_imageFilePath', 30, 8],
        ['m_campaignIndex', 10, 15],
        ['m_mapFileName', 30, 9],
        ['m_cacheHandles', 37, 10],
        ['m_miniSave', 13, 11],
        ['m_gameSpeed', 12, 12],
        ['m_defaultDifficulty', 3, 13],
        ['m_modPaths', 39, 14],
      ],
    },
    {
      k: 'optional',
      typeid: 9,
    },
    {
      k: 'optional',
      typeid: 35,
    },
    {
      k: 'optional',
      typeid: 6,
    },
    {
      k: 'struct',
      fields: [['m_race', 25, -1]],
    },
    {
      k: 'struct',
      fields: [['m_team', 25, -1]],
    },
    {
      k: 'blob',
      bounds: [0, 9],
    },
    {
      k: 'struct',
      fields: [
        ['m_name', 9, -18],
        ['m_clanTag', 41, -17],
        ['m_clanLogo', 42, -16],
        ['m_highestLeague', 25, -15],
        ['m_combinedRaceLevels', 43, -14],
        ['m_randomSeed', 6, -13],
        ['m_racePreference', 44, -12],
        ['m_teamPreference', 45, -11],
        ['m_testMap', 13, -10],
        ['m_testAuto', 13, -9],
        ['m_examine', 13, -8],
        ['m_customInterface', 13, -7],
        ['m_testType', 6, -6],
        ['m_observe', 24, -5],
        ['m_hero', 46, -4],
        ['m_skin', 46, -3],
        ['m_mount', 46, -2],
        ['m_toonHandle', 20, -1],
      ],
    },
    {
      k: 'array',
      bounds: [0, 5],
      typeid: 47,
    },
    {
      k: 'struct',
      fields: [
        ['m_lockTeams', 13, -15],
        ['m_teamsTogether', 13, -14],
        ['m_advancedSharedControl', 13, -13],
        ['m_randomRaces', 13, -12],
        ['m_battleNet', 13, -11],
        ['m_amm', 13, -10],
        ['m_competitive', 13, -9],
        ['m_practice', 13, -8],
        ['m_cooperative', 13, -7],
        ['m_noVictoryOrDefeat', 13, -6],
        ['m_heroDuplicatesAllowed', 13, -5],
        ['m_fog', 24, -4],
        ['m_observers', 24, -3],
        ['m_userDifficulty', 24, -2],
        ['m_clientDebugFlags', 21, -1],
      ],
    },
    {
      k: 'int',
      bounds: [1, 4],
    },
    {
      k: 'int',
      bounds: [1, 8],
    },
    {
      k: 'bitarray',
      bounds: [0, 6],
    },
    {
      k: 'bitarray',
      bounds: [0, 8],
    },
    {
      k: 'bitarray',
      bounds: [0, 2],
    },
    {
      k: 'bitarray',
      bounds: [0, 7],
    },
    {
      k: 'struct',
      fields: [
        ['m_allowedColors', 52, -6],
        ['m_allowedRaces', 53, -5],
        ['m_allowedDifficulty', 52, -4],
        ['m_allowedControls', 53, -3],
        ['m_allowedObserveTypes', 54, -2],
        ['m_allowedAIBuilds', 55, -1],
      ],
    },
    {
      k: 'array',
      bounds: [0, 5],
      typeid: 56,
    },
    {
      k: 'struct',
      fields: [
        ['m_randomValue', 6, -26],
        ['m_gameCacheName', 29, -25],
        ['m_gameOptions', 49, -24],
        ['m_gameSpeed', 12, -23],
        ['m_gameType', 12, -22],
        ['m_maxUsers', 2, -21],
        ['m_maxObservers', 2, -20],
        ['m_maxPlayers', 2, -19],
        ['m_maxTeams', 50, -18],
        ['m_maxColors', 3, -17],
        ['m_maxRaces', 51, -16],
        ['m_maxControls', 10, -15],
        ['m_mapSizeX', 10, -14],
        ['m_mapSizeY', 10, -13],
        ['m_mapFileSyncChecksum', 6, -12],
        ['m_mapFileName', 30, -11],
        ['m_mapAuthorName', 9, -10],
        ['m_modFileSyncChecksum', 6, -9],
        ['m_slotDescriptions', 57, -8],
        ['m_defaultDifficulty', 3, -7],
        ['m_defaultAIBuild', 0, -6],
        ['m_cacheHandles', 36, -5],
        ['m_hasExtensionMod', 13, -4],
        ['m_isBlizzardMap', 13, -3],
        ['m_isPremadeFFA', 13, -2],
        ['m_isCoopMode', 13, -1],
      ],
    },
    {
      k: 'optional',
      typeid: 1,
    },
    {
      k: 'optional',
      typeid: 2,
    },
    {
      k: 'struct',
      fields: [['m_color', 60, -1]],
    },
    {
      k: 'array',
      bounds: [0, 4],
      typeid: 46,
    },
    {
      k: 'array',
      bounds: [0, 17],
      typeid: 6,
    },
    {
      k: 'array',
      bounds: [0, 9],
      typeid: 6,
    },
    {
      k: 'struct',
      fields: [
        ['m_control', 10, -22],
        ['m_userId', 59, -21],
        ['m_teamId', 1, -20],
        ['m_colorPref', 61, -19],
        ['m_racePref', 44, -18],
        ['m_difficulty', 3, -17],
        ['m_aiBuild', 0, -16],
        ['m_handicap', 0, -15],
        ['m_observe', 24, -14],
        ['m_logoIndex', 6, -13],
        ['m_hero', 46, -12],
        ['m_skin', 46, -11],
        ['m_mount', 46, -10],
        ['m_artifacts', 62, -9],
        ['m_workingSetSlotId', 25, -8],
        ['m_rewards', 63, -7],
        ['m_toonHandle', 20, -6],
        ['m_licenses', 64, -5],
        ['m_tandemLeaderUserId', 59, -4],
        ['m_commander', 46, -3],
        ['m_commanderLevel', 6, -2],
        ['m_hasSilencePenalty', 13, -1],
      ],
    },
    {
      k: 'array',
      bounds: [0, 5],
      typeid: 65,
    },
    {
      k: 'struct',
      fields: [
        ['m_phase', 12, -11],
        ['m_maxUsers', 2, -10],
        ['m_maxObservers', 2, -9],
        ['m_slots', 66, -8],
        ['m_randomSeed', 6, -7],
        ['m_hostUserId', 59, -6],
        ['m_isSinglePlayer', 13, -5],
        ['m_pickedMapTag', 10, -4],
        ['m_gameDuration', 6, -3],
        ['m_defaultDifficulty', 3, -2],
        ['m_defaultAIBuild', 0, -1],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_userInitialData', 48, -3],
        ['m_gameDescription', 58, -2],
        ['m_lobbyState', 67, -1],
      ],
    },
    {
      k: 'struct',
      fields: [['m_syncLobbyState', 68, -1]],
    },
    {
      k: 'struct',
      fields: [['m_name', 20, -1]],
    },
    {
      k: 'blob',
      bounds: [0, 6],
    },
    {
      k: 'struct',
      fields: [['m_name', 71, -1]],
    },
    {
      k: 'struct',
      fields: [
        ['m_name', 71, -3],
        ['m_type', 6, -2],
        ['m_data', 20, -1],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_type', 6, -3],
        ['m_name', 71, -2],
        ['m_data', 34, -1],
      ],
    },
    {
      k: 'array',
      bounds: [0, 5],
      typeid: 10,
    },
    {
      k: 'struct',
      fields: [
        ['m_signature', 75, -2],
        ['m_toonHandle', 20, -1],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_gameFullyDownloaded', 13, -14],
        ['m_developmentCheatsEnabled', 13, -13],
        ['m_testCheatsEnabled', 13, -12],
        ['m_multiplayerCheatsEnabled', 13, -11],
        ['m_syncChecksummingEnabled', 13, -10],
        ['m_isMapToMapTransition', 13, -9],
        ['m_debugPauseEnabled', 13, -8],
        ['m_useGalaxyAsserts', 13, -7],
        ['m_platformMac', 13, -6],
        ['m_cameraFollow', 13, -5],
        ['m_baseBuildNum', 6, -4],
        ['m_buildNum', 6, -3],
        ['m_versionFlags', 6, -2],
        ['m_hotkeyProfile', 46, -1],
      ],
    },
    {
      k: 'struct',
      fields: [],
    },
    {
      k: 'int',
      bounds: [0, 16],
    },
    {
      k: 'struct',
      fields: [
        ['x', 79, -2],
        ['y', 79, -1],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_which', 12, -2],
        ['m_target', 80, -1],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_fileName', 30, -5],
        ['m_automatic', 13, -4],
        ['m_overwrite', 13, -3],
        ['m_name', 9, -2],
        ['m_description', 29, -1],
      ],
    },
    {
      k: 'int',
      bounds: [1, 32],
    },
    {
      k: 'struct',
      fields: [['m_sequence', 83, -1]],
    },
    {
      k: 'null',
    },
    {
      k: 'int',
      bounds: [0, 20],
    },
    {
      k: 'int',
      bounds: [-2147483648, 32],
    },
    {
      k: 'struct',
      fields: [
        ['x', 86, -3],
        ['y', 86, -2],
        ['z', 87, -1],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_targetUnitFlags', 79, -7],
        ['m_timer', 10, -6],
        ['m_tag', 6, -5],
        ['m_snapshotUnitLink', 79, -4],
        ['m_snapshotControlPlayerId', 59, -3],
        ['m_snapshotUpkeepPlayerId', 59, -2],
        ['m_snapshotPoint', 88, -1],
      ],
    },
    {
      k: 'choice',
      bounds: [0, 2],
      choices: {
        '0': ['None', 85],
        '1': ['TargetPoint', 88],
        '2': ['TargetUnit', 89],
      },
    },
    {
      k: 'struct',
      fields: [
        ['m_target', 90, -4],
        ['m_time', 87, -3],
        ['m_verb', 29, -2],
        ['m_arguments', 29, -1],
      ],
    },
    {
      k: 'struct',
      fields: [['m_data', 91, -1]],
    },
    {
      k: 'int',
      bounds: [0, 25],
    },
    {
      k: 'struct',
      fields: [
        ['m_abilLink', 79, -3],
        ['m_abilCmdIndex', 2, -2],
        ['m_abilCmdData', 25, -1],
      ],
    },
    {
      k: 'optional',
      typeid: 94,
    },
    {
      k: 'choice',
      bounds: [0, 2],
      choices: {
        '0': ['None', 85],
        '1': ['TargetPoint', 88],
        '2': ['TargetUnit', 89],
        '3': ['Data', 6],
      },
    },
    {
      k: 'struct',
      fields: [
        ['m_cmdFlags', 93, -6],
        ['m_abil', 95, -5],
        ['m_data', 96, -4],
        ['m_sequence', 83, -3],
        ['m_otherUnit', 43, -2],
        ['m_unitGroup', 43, -1],
      ],
    },
    {
      k: 'int',
      bounds: [0, 9],
    },
    {
      k: 'bitarray',
      bounds: [0, 9],
    },
    {
      k: 'array',
      bounds: [0, 9],
      typeid: 98,
    },
    {
      k: 'choice',
      bounds: [0, 2],
      choices: {
        '0': ['None', 85],
        '1': ['Mask', 99],
        '2': ['OneIndices', 100],
        '3': ['ZeroIndices', 100],
      },
    },
    {
      k: 'struct',
      fields: [
        ['m_unitLink', 79, -4],
        ['m_subgroupPriority', 10, -3],
        ['m_intraSubgroupPriority', 10, -2],
        ['m_count', 98, -1],
      ],
    },
    {
      k: 'array',
      bounds: [0, 9],
      typeid: 102,
    },
    {
      k: 'struct',
      fields: [
        ['m_subgroupIndex', 98, -4],
        ['m_removeMask', 101, -3],
        ['m_addSubgroups', 103, -2],
        ['m_addUnitTags', 64, -1],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_controlGroupId', 1, -2],
        ['m_delta', 104, -1],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_controlGroupIndex', 1, -3],
        ['m_controlGroupUpdate', 12, -2],
        ['m_mask', 101, -1],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_count', 98, -6],
        ['m_subgroupCount', 98, -5],
        ['m_activeSubgroupIndex', 98, -4],
        ['m_unitTagsChecksum', 6, -3],
        ['m_subgroupIndicesChecksum', 6, -2],
        ['m_subgroupsChecksum', 6, -1],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_controlGroupId', 1, -2],
        ['m_selectionSyncData', 107, -1],
      ],
    },
    {
      k: 'array',
      bounds: [0, 3],
      typeid: 87,
    },
    {
      k: 'struct',
      fields: [
        ['m_recipientId', 1, -2],
        ['m_resources', 109, -1],
      ],
    },
    {
      k: 'struct',
      fields: [['m_chatMessage', 29, -1]],
    },
    {
      k: 'int',
      bounds: [-128, 8],
    },
    {
      k: 'struct',
      fields: [
        ['x', 87, -3],
        ['y', 87, -2],
        ['z', 87, -1],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_beacon', 112, -9],
        ['m_ally', 112, -8],
        ['m_flags', 112, -7],
        ['m_build', 112, -6],
        ['m_targetUnitTag', 6, -5],
        ['m_targetUnitSnapshotUnitLink', 79, -4],
        ['m_targetUnitSnapshotUpkeepPlayerId', 112, -3],
        ['m_targetUnitSnapshotControlPlayerId', 112, -2],
        ['m_targetPoint', 113, -1],
      ],
    },
    {
      k: 'struct',
      fields: [['m_speed', 12, -1]],
    },
    {
      k: 'struct',
      fields: [['m_delta', 112, -1]],
    },
    {
      k: 'struct',
      fields: [
        ['x', 87, -2],
        ['y', 87, -1],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_point', 117, -4],
        ['m_unit', 6, -3],
        ['m_pingedMinimap', 13, -2],
        ['m_option', 87, -1],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_verb', 29, -2],
        ['m_arguments', 29, -1],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_alliance', 6, -2],
        ['m_control', 6, -1],
      ],
    },
    {
      k: 'struct',
      fields: [['m_unitTag', 6, -1]],
    },
    {
      k: 'struct',
      fields: [
        ['m_unitTag', 6, -2],
        ['m_flags', 10, -1],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_conversationId', 87, -2],
        ['m_replyId', 87, -1],
      ],
    },
    {
      k: 'optional',
      typeid: 20,
    },
    {
      k: 'struct',
      fields: [
        ['m_gameUserId', 1, -6],
        ['m_observe', 24, -5],
        ['m_name', 9, -4],
        ['m_toonHandle', 124, -3],
        ['m_clanTag', 41, -2],
        ['m_clanLogo', 42, -1],
      ],
    },
    {
      k: 'array',
      bounds: [0, 5],
      typeid: 125,
    },
    {
      k: 'int',
      bounds: [0, 1],
    },
    {
      k: 'struct',
      fields: [
        ['m_userInfos', 126, -2],
        ['m_method', 127, -1],
      ],
    },
    {
      k: 'struct',
      fields: [['m_purchaseItemId', 87, -1]],
    },
    {
      k: 'struct',
      fields: [['m_difficultyLevel', 87, -1]],
    },
    {
      k: 'choice',
      bounds: [0, 3],
      choices: {
        '0': ['None', 85],
        '1': ['Checked', 13],
        '2': ['ValueChanged', 6],
        '3': ['SelectionChanged', 87],
        '4': ['TextChanged', 30],
        '5': ['MouseButton', 6],
      },
    },
    {
      k: 'struct',
      fields: [
        ['m_controlId', 87, -3],
        ['m_eventType', 87, -2],
        ['m_eventData', 131, -1],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_soundHash', 6, -2],
        ['m_length', 6, -1],
      ],
    },
    {
      k: 'array',
      bounds: [0, 7],
      typeid: 6,
    },
    {
      k: 'struct',
      fields: [
        ['m_soundHash', 134, -2],
        ['m_length', 134, -1],
      ],
    },
    {
      k: 'struct',
      fields: [['m_syncInfo', 135, -1]],
    },
    {
      k: 'struct',
      fields: [
        ['m_queryId', 79, -3],
        ['m_lengthMs', 6, -2],
        ['m_finishGameLoop', 6, -1],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_queryId', 79, -2],
        ['m_lengthMs', 6, -1],
      ],
    },
    {
      k: 'struct',
      fields: [['m_animWaitQueryId', 79, -1]],
    },
    {
      k: 'struct',
      fields: [['m_sound', 6, -1]],
    },
    {
      k: 'struct',
      fields: [
        ['m_transmissionId', 87, -2],
        ['m_thread', 6, -1],
      ],
    },
    {
      k: 'struct',
      fields: [['m_transmissionId', 87, -1]],
    },
    {
      k: 'optional',
      typeid: 80,
    },
    {
      k: 'optional',
      typeid: 79,
    },
    {
      k: 'optional',
      typeid: 112,
    },
    {
      k: 'struct',
      fields: [
        ['m_target', 143, -6],
        ['m_distance', 144, -5],
        ['m_pitch', 144, -4],
        ['m_yaw', 144, -3],
        ['m_reason', 145, -2],
        ['m_follow', 13, -1],
      ],
    },
    {
      k: 'struct',
      fields: [['m_skipType', 127, -1]],
    },
    {
      k: 'int',
      bounds: [0, 11],
    },
    {
      k: 'struct',
      fields: [
        ['x', 148, -2],
        ['y', 148, -1],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_button', 6, -5],
        ['m_down', 13, -4],
        ['m_posUI', 149, -3],
        ['m_posWorld', 88, -2],
        ['m_flags', 112, -1],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_posUI', 149, -3],
        ['m_posWorld', 88, -2],
        ['m_flags', 112, -1],
      ],
    },
    {
      k: 'struct',
      fields: [['m_achievementLink', 79, -1]],
    },
    {
      k: 'struct',
      fields: [
        ['m_hotkey', 6, -2],
        ['m_down', 13, -1],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_abilLink', 79, -3],
        ['m_abilCmdIndex', 2, -2],
        ['m_state', 112, -1],
      ],
    },
    {
      k: 'struct',
      fields: [['m_soundtrack', 6, -1]],
    },
    {
      k: 'struct',
      fields: [['m_planetId', 87, -1]],
    },
    {
      k: 'struct',
      fields: [
        ['m_key', 112, -2],
        ['m_flags', 112, -1],
      ],
    },
    {
      k: 'struct',
      fields: [['m_resources', 109, -1]],
    },
    {
      k: 'struct',
      fields: [['m_fulfillRequestId', 87, -1]],
    },
    {
      k: 'struct',
      fields: [['m_cancelRequestId', 87, -1]],
    },
    {
      k: 'struct',
      fields: [
        ['m_error', 87, -2],
        ['m_abil', 95, -1],
      ],
    },
    {
      k: 'struct',
      fields: [['m_researchItemId', 87, -1]],
    },
    {
      k: 'struct',
      fields: [['m_mercenaryId', 87, -1]],
    },
    {
      k: 'struct',
      fields: [
        ['m_battleReportId', 87, -2],
        ['m_difficultyLevel', 87, -1],
      ],
    },
    {
      k: 'struct',
      fields: [['m_battleReportId', 87, -1]],
    },
    {
      k: 'int',
      bounds: [0, 19],
    },
    {
      k: 'struct',
      fields: [['m_decrementMs', 166, -1]],
    },
    {
      k: 'struct',
      fields: [['m_portraitId', 87, -1]],
    },
    {
      k: 'struct',
      fields: [['m_functionName', 20, -1]],
    },
    {
      k: 'struct',
      fields: [['m_result', 87, -1]],
    },
    {
      k: 'struct',
      fields: [['m_gameMenuItemIndex', 87, -1]],
    },
    {
      k: 'int',
      bounds: [-32768, 16],
    },
    {
      k: 'struct',
      fields: [
        ['m_wheelSpin', 172, -2],
        ['m_flags', 112, -1],
      ],
    },
    {
      k: 'struct',
      fields: [['m_purchaseCategoryId', 87, -1]],
    },
    {
      k: 'struct',
      fields: [['m_button', 79, -1]],
    },
    {
      k: 'struct',
      fields: [
        ['m_cutsceneId', 87, -2],
        ['m_bookmarkName', 20, -1],
      ],
    },
    {
      k: 'struct',
      fields: [['m_cutsceneId', 87, -1]],
    },
    {
      k: 'struct',
      fields: [
        ['m_cutsceneId', 87, -3],
        ['m_conversationLine', 20, -2],
        ['m_altConversationLine', 20, -1],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_cutsceneId', 87, -2],
        ['m_conversationLine', 20, -1],
      ],
    },
    {
      k: 'struct',
      fields: [['m_leaveReason', 1, -1]],
    },
    {
      k: 'struct',
      fields: [
        ['m_observe', 24, -7],
        ['m_name', 9, -6],
        ['m_toonHandle', 124, -5],
        ['m_clanTag', 41, -4],
        ['m_clanLogo', 42, -3],
        ['m_hijack', 13, -2],
        ['m_hijackCloneGameUserId', 59, -1],
      ],
    },
    {
      k: 'optional',
      typeid: 83,
    },
    {
      k: 'struct',
      fields: [
        ['m_state', 24, -2],
        ['m_sequence', 182, -1],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_sequence', 182, -2],
        ['m_target', 88, -1],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_sequence', 182, -2],
        ['m_target', 89, -1],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_catalog', 10, -4],
        ['m_entry', 79, -3],
        ['m_field', 9, -2],
        ['m_value', 9, -1],
      ],
    },
    {
      k: 'struct',
      fields: [['m_index', 6, -1]],
    },
    {
      k: 'struct',
      fields: [['m_shown', 13, -1]],
    },
    {
      k: 'struct',
      fields: [
        ['m_recipient', 12, -2],
        ['m_string', 30, -1],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_recipient', 12, -2],
        ['m_point', 117, -1],
      ],
    },
    {
      k: 'struct',
      fields: [['m_progress', 87, -1]],
    },
    {
      k: 'struct',
      fields: [['m_status', 24, -1]],
    },
    {
      k: 'struct',
      fields: [
        ['m_abilLink', 79, -3],
        ['m_abilCmdIndex', 2, -2],
        ['m_buttonLink', 79, -1],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_behaviorLink', 79, -2],
        ['m_buttonLink', 79, -1],
      ],
    },
    {
      k: 'choice',
      bounds: [0, 2],
      choices: {
        '0': ['None', 85],
        '1': ['Ability', 193],
        '2': ['Behavior', 194],
        '3': ['Vitals', 172],
      },
    },
    {
      k: 'struct',
      fields: [
        ['m_announcement', 195, -3],
        ['m_otherUnitTag', 6, -2],
        ['m_unitTag', 6, -1],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_scoreValueMineralsCurrent', 87, 0],
        ['m_scoreValueVespeneCurrent', 87, 1],
        ['m_scoreValueMineralsCollectionRate', 87, 2],
        ['m_scoreValueVespeneCollectionRate', 87, 3],
        ['m_scoreValueWorkersActiveCount', 87, 4],
        ['m_scoreValueMineralsUsedInProgressArmy', 87, 5],
        ['m_scoreValueMineralsUsedInProgressEconomy', 87, 6],
        ['m_scoreValueMineralsUsedInProgressTechnology', 87, 7],
        ['m_scoreValueVespeneUsedInProgressArmy', 87, 8],
        ['m_scoreValueVespeneUsedInProgressEconomy', 87, 9],
        ['m_scoreValueVespeneUsedInProgressTechnology', 87, 10],
        ['m_scoreValueMineralsUsedCurrentArmy', 87, 11],
        ['m_scoreValueMineralsUsedCurrentEconomy', 87, 12],
        ['m_scoreValueMineralsUsedCurrentTechnology', 87, 13],
        ['m_scoreValueVespeneUsedCurrentArmy', 87, 14],
        ['m_scoreValueVespeneUsedCurrentEconomy', 87, 15],
        ['m_scoreValueVespeneUsedCurrentTechnology', 87, 16],
        ['m_scoreValueMineralsLostArmy', 87, 17],
        ['m_scoreValueMineralsLostEconomy', 87, 18],
        ['m_scoreValueMineralsLostTechnology', 87, 19],
        ['m_scoreValueVespeneLostArmy', 87, 20],
        ['m_scoreValueVespeneLostEconomy', 87, 21],
        ['m_scoreValueVespeneLostTechnology', 87, 22],
        ['m_scoreValueMineralsKilledArmy', 87, 23],
        ['m_scoreValueMineralsKilledEconomy', 87, 24],
        ['m_scoreValueMineralsKilledTechnology', 87, 25],
        ['m_scoreValueVespeneKilledArmy', 87, 26],
        ['m_scoreValueVespeneKilledEconomy', 87, 27],
        ['m_scoreValueVespeneKilledTechnology', 87, 28],
        ['m_scoreValueFoodUsed', 87, 29],
        ['m_scoreValueFoodMade', 87, 30],
        ['m_scoreValueMineralsUsedActiveForces', 87, 31],
        ['m_scoreValueVespeneUsedActiveForces', 87, 32],
        ['m_scoreValueMineralsFriendlyFireArmy', 87, 33],
        ['m_scoreValueMineralsFriendlyFireEconomy', 87, 34],
        ['m_scoreValueMineralsFriendlyFireTechnology', 87, 35],
        ['m_scoreValueVespeneFriendlyFireArmy', 87, 36],
        ['m_scoreValueVespeneFriendlyFireEconomy', 87, 37],
        ['m_scoreValueVespeneFriendlyFireTechnology', 87, 38],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_playerId', 1, 0],
        ['m_stats', 197, 1],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_unitTagIndex', 6, 0],
        ['m_unitTagRecycle', 6, 1],
        ['m_unitTypeName', 29, 2],
        ['m_controlPlayerId', 1, 3],
        ['m_upkeepPlayerId', 1, 4],
        ['m_x', 10, 5],
        ['m_y', 10, 6],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_unitTagIndex', 6, 0],
        ['m_unitTagRecycle', 6, 1],
        ['m_x', 10, 2],
        ['m_y', 10, 3],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_unitTagIndex', 6, 0],
        ['m_unitTagRecycle', 6, 1],
        ['m_killerPlayerId', 59, 2],
        ['m_x', 10, 3],
        ['m_y', 10, 4],
        ['m_killerUnitTagIndex', 43, 5],
        ['m_killerUnitTagRecycle', 43, 6],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_unitTagIndex', 6, 0],
        ['m_unitTagRecycle', 6, 1],
        ['m_controlPlayerId', 1, 2],
        ['m_upkeepPlayerId', 1, 3],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_unitTagIndex', 6, 0],
        ['m_unitTagRecycle', 6, 1],
        ['m_unitTypeName', 29, 2],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_playerId', 1, 0],
        ['m_upgradeTypeName', 29, 1],
        ['m_count', 87, 2],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_unitTagIndex', 6, 0],
        ['m_unitTagRecycle', 6, 1],
      ],
    },
    {
      k: 'array',
      bounds: [0, 10],
      typeid: 87,
    },
    {
      k: 'struct',
      fields: [
        ['m_firstUnitIndex', 6, 0],
        ['m_items', 206, 1],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_playerId', 1, 0],
        ['m_type', 6, 1],
        ['m_userId', 43, 2],
        ['m_slotId', 43, 3],
      ],
    },
    {
      k: 'struct',
      fields: [['m_key', 29, 0]],
    },
    {
      k: 'struct',
      fields: [
        ['__parent', 209, 0],
        ['m_value', 29, 1],
      ],
    },
    {
      k: 'array',
      bounds: [0, 6],
      typeid: 210,
    },
    {
      k: 'optional',
      typeid: 211,
    },
    {
      k: 'struct',
      fields: [
        ['__parent', 209, 0],
        ['m_value', 87, 1],
      ],
    },
    {
      k: 'array',
      bounds: [0, 6],
      typeid: 213,
    },
    {
      k: 'optional',
      typeid: 214,
    },
    {
      k: 'struct',
      fields: [
        ['m_eventName', 29, 0],
        ['m_stringData', 212, 1],
        ['m_intData', 215, 2],
        ['m_fixedData', 215, 3],
      ],
    },
    {
      k: 'struct',
      fields: [
        ['m_value', 6, 0],
        ['m_time', 6, 1],
      ],
    },
    {
      k: 'array',
      bounds: [0, 6],
      typeid: 217,
    },
    {
      k: 'array',
      bounds: [0, 5],
      typeid: 218,
    },
    {
      k: 'struct',
      fields: [
        ['m_name', 29, 0],
        ['m_values', 219, 1],
      ],
    },
    {
      k: 'array',
      bounds: [0, 21],
      typeid: 220,
    },
    {
      k: 'struct',
      fields: [['m_instanceList', 221, 0]],
    },
  ],
  gameEventTypes: {
    '5': [78, 'NNet.Game.SUserFinishedLoadingSyncEvent'],
    '7': [77, 'NNet.Game.SUserOptionsEvent'],
    '9': [70, 'NNet.Game.SBankFileEvent'],
    '10': [72, 'NNet.Game.SBankSectionEvent'],
    '11': [73, 'NNet.Game.SBankKeyEvent'],
    '12': [74, 'NNet.Game.SBankValueEvent'],
    '13': [76, 'NNet.Game.SBankSignatureEvent'],
    '14': [81, 'NNet.Game.SCameraSaveEvent'],
    '21': [82, 'NNet.Game.SSaveGameEvent'],
    '22': [78, 'NNet.Game.SSaveGameDoneEvent'],
    '23': [78, 'NNet.Game.SLoadGameDoneEvent'],
    '25': [84, 'NNet.Game.SCommandManagerResetEvent'],
    '26': [92, 'NNet.Game.SGameCheatEvent'],
    '27': [97, 'NNet.Game.SCmdEvent'],
    '28': [105, 'NNet.Game.SSelectionDeltaEvent'],
    '29': [106, 'NNet.Game.SControlGroupUpdateEvent'],
    '30': [108, 'NNet.Game.SSelectionSyncCheckEvent'],
    '31': [110, 'NNet.Game.SResourceTradeEvent'],
    '32': [111, 'NNet.Game.STriggerChatMessageEvent'],
    '33': [114, 'NNet.Game.SAICommunicateEvent'],
    '34': [115, 'NNet.Game.SSetAbsoluteGameSpeedEvent'],
    '35': [116, 'NNet.Game.SAddAbsoluteGameSpeedEvent'],
    '36': [118, 'NNet.Game.STriggerPingEvent'],
    '37': [119, 'NNet.Game.SBroadcastCheatEvent'],
    '38': [120, 'NNet.Game.SAllianceEvent'],
    '39': [121, 'NNet.Game.SUnitClickEvent'],
    '40': [122, 'NNet.Game.SUnitHighlightEvent'],
    '41': [123, 'NNet.Game.STriggerReplySelectedEvent'],
    '43': [128, 'NNet.Game.SHijackReplayGameEvent'],
    '44': [78, 'NNet.Game.STriggerSkippedEvent'],
    '45': [133, 'NNet.Game.STriggerSoundLengthQueryEvent'],
    '46': [140, 'NNet.Game.STriggerSoundOffsetEvent'],
    '47': [141, 'NNet.Game.STriggerTransmissionOffsetEvent'],
    '48': [142, 'NNet.Game.STriggerTransmissionCompleteEvent'],
    '49': [146, 'NNet.Game.SCameraUpdateEvent'],
    '50': [78, 'NNet.Game.STriggerAbortMissionEvent'],
    '51': [129, 'NNet.Game.STriggerPurchaseMadeEvent'],
    '52': [78, 'NNet.Game.STriggerPurchaseExitEvent'],
    '53': [130, 'NNet.Game.STriggerPlanetMissionLaunchedEvent'],
    '54': [78, 'NNet.Game.STriggerPlanetPanelCanceledEvent'],
    '55': [132, 'NNet.Game.STriggerDialogControlEvent'],
    '56': [136, 'NNet.Game.STriggerSoundLengthSyncEvent'],
    '57': [147, 'NNet.Game.STriggerConversationSkippedEvent'],
    '58': [150, 'NNet.Game.STriggerMouseClickedEvent'],
    '59': [151, 'NNet.Game.STriggerMouseMovedEvent'],
    '60': [152, 'NNet.Game.SAchievementAwardedEvent'],
    '61': [153, 'NNet.Game.STriggerHotkeyPressedEvent'],
    '62': [154, 'NNet.Game.STriggerTargetModeUpdateEvent'],
    '63': [78, 'NNet.Game.STriggerPlanetPanelReplayEvent'],
    '64': [155, 'NNet.Game.STriggerSoundtrackDoneEvent'],
    '65': [156, 'NNet.Game.STriggerPlanetMissionSelectedEvent'],
    '66': [157, 'NNet.Game.STriggerKeyPressedEvent'],
    '67': [169, 'NNet.Game.STriggerMovieFunctionEvent'],
    '68': [78, 'NNet.Game.STriggerPlanetPanelBirthCompleteEvent'],
    '69': [78, 'NNet.Game.STriggerPlanetPanelDeathCompleteEvent'],
    '70': [158, 'NNet.Game.SResourceRequestEvent'],
    '71': [159, 'NNet.Game.SResourceRequestFulfillEvent'],
    '72': [160, 'NNet.Game.SResourceRequestCancelEvent'],
    '73': [78, 'NNet.Game.STriggerResearchPanelExitEvent'],
    '74': [78, 'NNet.Game.STriggerResearchPanelPurchaseEvent'],
    '75': [162, 'NNet.Game.STriggerResearchPanelSelectionChangedEvent'],
    '76': [161, 'NNet.Game.STriggerCommandErrorEvent'],
    '77': [78, 'NNet.Game.STriggerMercenaryPanelExitEvent'],
    '78': [78, 'NNet.Game.STriggerMercenaryPanelPurchaseEvent'],
    '79': [163, 'NNet.Game.STriggerMercenaryPanelSelectionChangedEvent'],
    '80': [78, 'NNet.Game.STriggerVictoryPanelExitEvent'],
    '81': [78, 'NNet.Game.STriggerBattleReportPanelExitEvent'],
    '82': [164, 'NNet.Game.STriggerBattleReportPanelPlayMissionEvent'],
    '83': [165, 'NNet.Game.STriggerBattleReportPanelPlaySceneEvent'],
    '84': [165, 'NNet.Game.STriggerBattleReportPanelSelectionChangedEvent'],
    '85': [130, 'NNet.Game.STriggerVictoryPanelPlayMissionAgainEvent'],
    '86': [78, 'NNet.Game.STriggerMovieStartedEvent'],
    '87': [78, 'NNet.Game.STriggerMovieFinishedEvent'],
    '88': [167, 'NNet.Game.SDecrementGameTimeRemainingEvent'],
    '89': [168, 'NNet.Game.STriggerPortraitLoadedEvent'],
    '90': [170, 'NNet.Game.STriggerCustomDialogDismissedEvent'],
    '91': [171, 'NNet.Game.STriggerGameMenuItemSelectedEvent'],
    '92': [173, 'NNet.Game.STriggerMouseWheelEvent'],
    '93': [129, 'NNet.Game.STriggerPurchasePanelSelectedPurchaseItemChangedEvent'],
    '94': [174, 'NNet.Game.STriggerPurchasePanelSelectedPurchaseCategoryChangedEvent'],
    '95': [175, 'NNet.Game.STriggerButtonPressedEvent'],
    '96': [78, 'NNet.Game.STriggerGameCreditsFinishedEvent'],
    '97': [176, 'NNet.Game.STriggerCutsceneBookmarkFiredEvent'],
    '98': [177, 'NNet.Game.STriggerCutsceneEndSceneFiredEvent'],
    '99': [178, 'NNet.Game.STriggerCutsceneConversationLineEvent'],
    '100': [179, 'NNet.Game.STriggerCutsceneConversationLineMissingEvent'],
    '101': [180, 'NNet.Game.SGameUserLeaveEvent'],
    '102': [181, 'NNet.Game.SGameUserJoinEvent'],
    '103': [183, 'NNet.Game.SCommandManagerStateEvent'],
    '104': [184, 'NNet.Game.SCmdUpdateTargetPointEvent'],
    '105': [185, 'NNet.Game.SCmdUpdateTargetUnitEvent'],
    '106': [137, 'NNet.Game.STriggerAnimLengthQueryByNameEvent'],
    '107': [138, 'NNet.Game.STriggerAnimLengthQueryByPropsEvent'],
    '108': [139, 'NNet.Game.STriggerAnimOffsetEvent'],
    '109': [186, 'NNet.Game.SCatalogModifyEvent'],
    '110': [187, 'NNet.Game.SHeroTalentTreeSelectedEvent'],
    '111': [78, 'NNet.Game.STriggerProfilerLoggingFinishedEvent'],
    '112': [188, 'NNet.Game.SHeroTalentTreeSelectionPanelToggledEvent'],
  },
  messageEventTypes: {
    '0': [189, 'NNet.Game.SChatMessage'],
    '1': [190, 'NNet.Game.SPingMessage'],
    '2': [191, 'NNet.Game.SLoadingProgressMessage'],
    '3': [78, 'NNet.Game.SServerPingMessage'],
    '4': [192, 'NNet.Game.SReconnectNotifyMessage'],
    '5': [196, 'NNet.Game.SPlayerAnnounceMessage'],
  },
  trackerEventTypes: {
    '0': [198, 'NNet.Replay.Tracker.SPlayerStatsEvent'],
    '1': [199, 'NNet.Replay.Tracker.SUnitBornEvent'],
    '2': [201, 'NNet.Replay.Tracker.SUnitDiedEvent'],
    '3': [202, 'NNet.Replay.Tracker.SUnitOwnerChangeEvent'],
    '4': [203, 'NNet.Replay.Tracker.SUnitTypeChangeEvent'],
    '5': [204, 'NNet.Replay.Tracker.SUpgradeEvent'],
    '6': [199, 'NNet.Replay.Tracker.SUnitInitEvent'],
    '7': [205, 'NNet.Replay.Tracker.SUnitDoneEvent'],
    '8': [207, 'NNet.Replay.Tracker.SUnitPositionsEvent'],
    '9': [208, 'NNet.Replay.Tracker.SPlayerSetupEvent'],
    '10': [216, 'NNet.Replay.Tracker.SStatGameEvent'],
    '11': [222, 'NNet.Replay.Tracker.SScoreResultEvent'],
    '12': [200, 'NNet.Replay.Tracker.SUnitRevivedEvent'],
  },
  gameEventIdTypeid: 0,
  messageEventIdTypeid: 1,
  trackerEventIdTypeid: 2,
  svaruint32Typeid: 7,
  replayUserIdTypeid: 8,
  headerTypeid: 18,
  detailsTypeid: 40,
  initDataTypeid: 69,
};

export default protocol;
